...
async function callAPI(endpoint, data) {
    ...
    if (!response.ok) {
        if (contentType.includes('application/json')) {
            ...
            throw new Error(error.detail || `Hata ${response.status}: Bir sorun oluştu`);
        } else {
            const text = await response.text();
            throw new Error(`Sunucu hatası (${response.status}): API bağlantısı kurulamadı. Backend çalışıyor mu?`);
        }
    }

    if (!contentType.includes('application/json')) {
        throw new Error('Sunucu geçersiz yanıt döndürdü. API bağlantısı kurulamadı.');
    }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (ct.includes('application/json')) {
                ...
                throw new Error(`Sunucu hatası (${response.status}): API bağlantısı kurulamadı.`);
            }
        }
